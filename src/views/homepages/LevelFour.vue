...
const productsList = async (page = 1) => {
    products.value = [];
    let params = {
        level_two_tag: tagStore.getLevelTwoTagId ?? route.params.levelTwoslug,
        level_three_tag: tagStore.getLevelThreeTagId ?? route.params.levelThreeslug,
...
const handleSearch = async (data) => {
    if (data) {
        ...
        if (data.priceOrder !== undefined) {
            priceOrder.value = data.priceOrder
        }
        productsList()
    }
}
...
const fetchFilters = async () => {
    await getProductFilters(tagStore.getLevelOneTagId, {
        level_two_tag: tagStore.getLevelTwoTagSlug ?? route.params.levelTwoslug,
        level_three_tag: tagStore.getLevelThreeTagSlug ?? route.params.levelThreeslug,
        level_four_tag: route.params.levelFourslug !== 'all' ? route.params.levelFourslug : null,
        filters: filter.value
    });
}
...
// transforms filters
const transform = (filters, index = null) => {
...
<template>
    <div v-if="itemContainerValidator('filters')" class="row">
        ...
        <div class="my-2" v-if="selectedFilters">
            <button type="button" class="btn btn-primary ms-2 cursor-text" v-for="(filter, index) in selectedFilters"
                :key="index">
                <span v-if="filter.min && filter.max">{{ `${formatCurrency(filter.min)} - ${formatCurrency(filter.max)}`
                    }}</span>
        ...
        <AttributeFilters ref="filterComponent" v-for="(filters, index) in productFilters"
            :attributeFilter="transform(filters, index)" :attributeName="index" @filter-attributes="setFilters"
    ...
    <ItemContainer v-if="!itemContainerValidator('horizontalProductsLayout')">
        <template #content>
            ...
            <div v-else>
                <div class="row">
                    <div class="col-lg-3 col-xxl-3 col-md-6 col-sm-12 mb-4" v-for="(data, index) in products"
                        :key="index">
    ...
    <div v-else>
        <HorizontalProductsLayout :levelTwoTag="tagStore.getLevelTwoTagSlug"
            :levelThreeTag="tagStore.getLevelThreeTagSlug" :levelFourTag="tagStore.getLevelFourTagSlug" />
    </div>
</template>